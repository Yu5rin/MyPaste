...
3. `RELEASE_NOTES.md` をそのバージョンの変更点だけに書き換える（GitHub Release の本文に使う。形は下記）
4. `main` にコミット・プッシュ
...
- **README に置く**: アプリの説明・画面写真・主な特徴・動作環境・インストール・更新方法・使い方・ビルド。
  版をまたいで変わらない説明はすべて README に書く（書くのは現在の事実だけ）。
- **リリース本文**の `##` 見出しは `## 変更点` と `## ダウンロード`（ファイル・サイズ・SHA256 の表）の 2 つだけ。
  最後に `インストールと更新の方法は [README](https://github.com/Yu5rin/MyPaste#インストール) をご覧ください。`
  の 1 行を置く。その版の変更の小見出しは `###` 以下にする。
- `# アタイの貼り付け vX.Y.Z` のような題名行は書かない。リリースのタイトルはタグと同じ表記（例 `v1.2.3`）。
- その版に上げるときだけ必要な注意（手で入れ替える手順など）は、`###` として変更点の中に書く。
- 前の版の変更点を積み重ねない。その版の分だけを書く。
- `RELEASE_NOTES.md` には `## 変更点` だけを書く。Release ワークフロー（`.github/workflows/release.yml`）が
  ダウンロードの表と README への案内を足して本文を作る。手動でリリースを作るときは、この 2 つを自分で足す。
- 自動更新はリリース本文を読まない（SHA256 は添付ファイルの `digest` から取る。`src/update.rs`）。
